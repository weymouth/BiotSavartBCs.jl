...
using BiotSavartBCs: @loop2,inside_u,restrict!,project!,down,front,step
@testset "util.jl" begin
    a = zeros(Int,(4,4,6,3))
    @loop2 a[I] += 1 over I in inside_u(a,buff=2)
    @test sum(a) == 0
    @loop2 a[I] += 1 over I in inside_u(a)
    ...
    @loop2 ml[3][I] += 4 over I in tar[3]
    project!(ml,tar)
    @test ml[2][Ti] == 1
