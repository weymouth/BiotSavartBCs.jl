...
let
    ...
    plt = plot(dpi=300);
    for (i,θ) ∈ enumerate(params)
        file = jldopen("domain_$(θ[1])_$(θ[2])_$(θ[3]).jld2")
    ...
    for (i,θ) ∈ enumerate(params)
        ...
        plot!(plt,range(0,2.5,length=50),abs.(u_hat[1:50]),label="$(n)Dx$(m)D using "*BC;ls,c=col)
    end
    ...
    file = jldopen("rotated_circle_7Dx7D.jld2", "r")
    f = Float32.(reduce(vcat,file["f"]')[1:end,:])
